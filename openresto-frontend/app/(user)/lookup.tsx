        ...
        <View style={isWide ? styles.wideRow : undefined}>
          ...
          <View style={isWide ? styles.wideCol : undefined}>
            {!loading && searched && !booking && (
              <View style={[styles.resultCard, { backgroundColor: colors.card, borderColor }, isWide ? { marginTop: 0 } : { marginTop: 16 }]}>
...
function SearchForm({ refInput, emailInput, onRefChange, onEmailChange, onSubmit, canSearch, loading, accent, colors, borderColor }: {
  refInput: string; emailInput: string; onRefChange: (v: string) => void; onEmailChange: (v: string) => void;
  onSubmit: () => void; canSearch: boolean; loading: boolean; accent: string;
  colors: ReturnType<typeof getThemeColors>; borderColor: string;
}) {
  return (
    <View style={[styles.searchCard, { backgroundColor: colors.card, borderColor }]}>
      ...
      <Input placeholder="e.g. crispy-basil-thyme" value={refInput} onChangeText={onRefChange} autoCapitalize="none" />
      <ThemedText style={styles.label}>Email Address</ThemedText>
      <Input placeholder="The email used when booking" value={emailInput} onChangeText={onEmailChange} autoCapitalize="none" keyboardType="email-address" returnKeyType="go" onSubmitEditing={onSubmit} />
      <Pressable onPress={onSubmit} disabled={!canSearch || loading} style={[styles.searchBtn, { backgroundColor: accent }, (!canSearch || loading) && { opacity: 0.5 }]}>
        {loading ? (
...
function RecentBookings({ cached, colors, borderColor, style, onSelect }: {
  cached: CachedBooking[]; colors: ReturnType<typeof getThemeColors>; borderColor: string;
  style?: object; onSelect: (c: CachedBooking) => void;
}) {
  if (cached.length === 0) return null;
  return (
    <View style={[styles.recentSection, style]}>
      <ThemedText style={[styles.recentTitle, { color: colors.muted }]}>YOUR RECENT BOOKINGS</ThemedText>
      {cached.map((c) => (
        <Pressable key={c.bookingRef} style={[styles.recentCard, { backgroundColor: colors.card, borderColor }]} onPress={() => onSelect(c)}>
...
function BookingResult({ booking, restaurant, isDark, accent, colors, borderColor, isWide, onCancel }: {
  booking: BookingDto; restaurant: RestaurantDto | null; isDark: boolean; accent: string;
  colors: ReturnType<typeof getThemeColors>; borderColor: string; isWide: boolean; onCancel: () => void;
}) {
  return (
    <View style={[styles.detailCard, { backgroundColor: colors.card, borderColor }, isWide ? {} : { marginTop: 24 }]}>
      <View style={styles.cardHeader}>
        <View style={styles.resultHeader}>
          <Ionicons name={booking.isCancelled ? "close-circle" : "checkmark-circle"} size={20} color={booking.isCancelled ? COLORS.error : "#16a34a"} />
      ...
      <BookingDetailRows booking={booking} restaurant={restaurant} mutedColor={colors.muted} borderColor={borderColor} />
...
const styles = StyleSheet.create({
  ...
  wideCol: { flex: 1 },
  searchCard: { borderRadius: 14, borderWidth: 1, padding: 20, gap: 12, width: "100%", shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 2 },
  label: { fontSize: 13, fontWeight: "600", letterSpacing: 0.2 },
  searchBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, ...BUTTON_SIZES.primary, borderRadius: 10, marginTop: 4 },
  searchBtnText: { color: "#fff", fontSize: 16, fontWeight: "700" },
  helpText: { fontSize: 12, textAlign: "center", lineHeight: 18, marginTop: 4 },
  resultCard: { borderRadius: 14, borderWidth: 1, padding: 24, marginTop: 20, width: "100%", alignItems: "center", gap: 16, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 2 },
  ...
  cardHeader: { alignItems: "center", gap: 10, paddingHorizontal: 16, paddingTop: 20, paddingBottom: 16 },
  ...
  divider: { height: 1 },
  cancelBtn: { flexDirection: "row", alignItems: "center", gap: 12, paddingHorizontal: 16, paddingVertical: 14 },
  cancelBtnText: { color: COLORS.error, fontSize: 15, fontWeight: "600" },
  cancelledContent: { flexDirection: "row", alignItems: "center", gap: 12, paddingHorizontal: 16, paddingVertical: 14 },
  ...
  recentCard: { borderRadius: 12, borderWidth: 1, padding: 14, shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.04, shadowRadius: 4, elevation: 1 },
