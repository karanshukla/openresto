...
- **`scripts/demo_data.py` is the single source of truth for seed/demo data.** It declares the dataset and emits SQL (`config` / `bookings` / `media` / `accounts` / `all`); `seed-local.sh` (local dev DB) and `purge-bookings.sh` (the demo VPS's 2-hourly reset) are both thin wrappers over it. This replaced three hand-maintained copies of the same dataset that had drifted apart, so **add new data here, never inline in a shell script** — and when adding a column, add it to the dataset too or the demo silently stops exercising it. The dataset deliberately covers every feature flag the product has (per-day hours incl. a past-midnight wrap, both booking-ref formats, all contact-fallback states, 15/30/60 slot intervals, walk-in-only both global and per-day, a live booking pause, an archived location, the table-oversize cap, and named/unnamed combinable groups) — each location is annotated with the feature it exists to surface. The `accounts` section covers the multi-user surface the locations can't: an Owner (no display name) and a Manager (one), so both roles and both branches of the display-name fallback are there to click through. It is **not** part of `all` — it needs an admin password, taken from `ADMIN_EMAIL`/`ADMIN_PASSWORD` or `--settings-file`, never from argv where `ps` would expose it — and it wipes the table first, so an account a demo visitor invited themselves doesn't survive the reset. Each section emits its own `BEGIN`/`COMMIT`, so callers apply `accounts` as a separate batch rather than concatenating it. Bookings are generated against the same rules the server enforces (opening hours, capacity, oversize cap, and the table↔group mutual exclusion in `IsUnitBookedOnDateAsync`), so seeded rows never conflict with each other and always carry an `EndTime` like real ones do.
