...
- **Unistyles spike (issue #219, not adopted)**: a proof of concept lives on the branch documented in `docs/unistyles-spike.md`. It is why `babel.config.js` and `index.ts` exist and why `main` in `package.json` is no longer `expo-router/entry`. Three constraints matter if you touch it: (1) any file calling Unistyles' `StyleSheet.create` must first side-effect-import `@/theme/unistyles`, because the static web export's manifest pass loads route modules without ever evaluating the app entry; (2) `react-native-web` injects its reset stylesheet after Unistyles', so `moveUnistylesStylesLast()` in `hooks/use-unistyles-bridge.ts` is load-bearing — remove it and the app silently renders unstyled on web; (3) passing a Unistyles style through RN's `StyleSheet.flatten` discards it, which is why `themed-text.tsx` no longer flattens. The recommendation in the doc is to stay on the bespoke system: the app is web-only in practice, which is where Unistyles has the least to offer.
