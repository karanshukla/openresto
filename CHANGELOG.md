...
- **Multiple admin accounts, with Owner and Manager roles** (#265) — OpenResto supported exactly one admin per instance, and that assumption was welded into the data model, the auth pipeline and the UI. `AdminCredentials` is no longer a singleton table: each row is a user with a display name, a role, an active flag, and its own security question, and login looks the account up by email rather than comparing the supplied address against whichever row came back first. JWTs now carry the user id and the account's real role instead of a hardcoded `Admin`, so every self-service action (change password, change email, set a security question) targets the person who made the request rather than a global record — the bug that made a second account impossible even once one existed. Owners get a Users card in settings to invite colleagues with a temporary password, promote or demote them, deactivate and reactivate them, and set a new password for someone locked out; Managers get everything else and never see the card. The instance is protected from being locked out of itself: the last active Owner cannot be demoted or deactivated, and nobody can deactivate their own account or change their own role — handing over ownership takes the other Owner. Role values live in one allow-list and controllers gate through named policies (`RequireAdmin`, `RequireOwner`) rather than raw role strings, so adding a role later is a one-file change; the frontend mirrors that with a single `useCan(capability)` seam. Upgrading is a no-op for existing installs — the migration is additive, the single existing admin becomes the Owner, and sessions started before the upgrade keep working until they expire rather than being signed out mid-shift. The demo dataset seeds both roles, so the public demo and a freshly seeded dev database both have a Manager to sign in as and watch the Users card disappear.

### Fixed

- **Notification rows rendered with red blocks down both edges.** `ReanimatedSwipeable` lays its swipe-to-delete panels behind the row, and the row never painted a background of its own, so on a pointer device the panels simply showed through as red bleed at either end of every notification. The row now paints the card surface, and the panels are only rendered where a swipe is actually possible (touch input, unpinned rows) rather than sitting in the layout unused.
- **Upgrading to the multi-user schema failed on any instance that already had an admin account.** The `AddMultiUserAccounts` migration added `CreatedAt` with a `CURRENT_TIMESTAMP` default, which SQLite permits only while the table is empty and rejects with `Cannot add a column with non-constant default` once it holds a row. A fresh install migrates before anything is written, so the only databases that hit it were real ones. The column is now added nullable, backfilled, and the table rebuilt to reach the same definition, and the migration-safety CI job seeds a row into every table before applying new migrations so an empty-database pass can no longer stand in for a working upgrade.

### Changed

- **Settings is four pages now, not one scroll** (#322). Brand, footer, highlights, SMTP, push, account security and user management were all stacked on `/admin/settings`, so finding anything meant scrolling past everything else, and adding the Users card in #265 made a long page longer. The route splits into `/admin/settings/brand` (identity, footer, highlights), `/admin/settings/email` (SMTP, guest confirmations, push), `/admin/settings/users` (Owner only) and `/admin/settings/account` (your own password, email and security question). `/admin/settings` redirects to Brand, so old links and the `g s` shortcut still land somewhere sensible. The sidebar splits to match: MANAGE for the day-to-day routes, CONFIGURE for the settings ones, with Users only rendered for a role that can actually manage users. The nav is taller than it was, so the sidebar now scrolls on its own instead of clipping the log-out row on a short screen.
- **Settings UI cleanup.** The settings routes now share the same 1200px page width as every other admin route (Locations, Bookings, Dashboard) instead of a narrower 880px column, so switching between CONFIGURE and MANAGE no longer feels like the page shrank. Related fields that used to stack full-width — contact phone/email, highlights heading/subheading, the security-question and password-change forms, the invite-a-user email/display-name pair — now sit side by side in a row that drops back to stacked on a narrow screen, so the extra width goes toward laying fields out rather than stretching a single input edge to edge. In the Users card the role picker sat outside the account tile it belonged to, looking like an orphaned pair of buttons: it moves inside the tile under a "Role" label, the "Add user" button stops spanning the card, and an account with no display name no longer prints its email twice. The sidebar identity shows the part of the email before the @ rather than a truncated address. Copy in the admin and booking flows drops its dashes for full sentences.
- **Row actions in the admin are named buttons, not bare glyphs.** Delete, Combine, Edit, Reset password, Deactivate and Reactivate all carry their label now, in the sections and tables list, the social links and highlights lists, the notification rows and the Users card. The admin runs on tablets, often for staff who are not in it every day, and a trash can next to a link icon next to a key is a puzzle rather than a control. Directional move-up/move-down arrows stay as icons, since an arrow is its own label.
