...
public class BrandServiceTests
{
    ...
    {
        using AppDbContext db = CreateDb(nameof(SaveAsync_Persists_CopyrightText));
        var svc = CreateService(db);
    ...
    {
        using AppDbContext db = CreateDb(nameof(SaveAsync_Clears_CopyrightText_WhenEmptyStringPassed));
        var svc = CreateService(db);
        await svc.SaveAsync(null, null, null, copyrightText: "© 2026 My Resto");
